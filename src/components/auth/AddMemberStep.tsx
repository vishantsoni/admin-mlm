...
export default function AddMemberStep() {
    ...
    const [downlines, setDownlines] = useState<{ id: string, name: string }[]>([]);
    const [formData, setFormData] = useState({
        ...
        p_id: '', // 1] - Store selected parent ID instead of referral code
        referralCode: '', // 4]
        referrer_id: '',
        referrerName: '', // 4]
        referrerContact: '', // 5]
    ...
    console.log("Reffer code - ", formData);
    ...
    const fetchDownline = useCallback(async () => {
        try {
            ...
            if (res.success) {
                setDownlines([...res.data, { id: user?.id, name: "My Self", referral_code: user?.referral_code }]);
            }
        } catch (error) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (response.status == true) {
                ...
                setFieldErrors({ error: `Submission failed: ${errorData.message || 'Unknown error'}` });
            }
                ...
                {step === 1 && (
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 md:col-span-2">
                                <div>
                                    ...
                                    <select
                                        ...
                                        onChange={(e) => {
                                            ...
                                            const selectedDownline = downlines.find(d => parseInt(d.id) === parseInt(e.target.value));
                                            // console.log("selected downline = ", downlines, selectedDownline, e.target.value);


                                            setFormData({
                                                ...formData,
                                                p_id: e.target.value,
                                                referralCode: selectedDownline?.referral_code || "",
                                                referrerName: selectedDownline?.name || ""
                                            })
                                        }}
                                    >
                                        <option value={user?.id}>My Self</option>
                                        {
                                            downlines.length > 0 ? downlines.map((d) => (
                                                <option key={d.id} value={d.id}>{d.name}</option>
                                            )) : <option disabled>No downlines found</option>
                                        }

                                    </select>
                                    <span className="text-xs text-gray-400 mt-1">Choose where this new member will be placed in your tree. Select 'My Self' for a direct referral.</span>
                                </div>
                                <div>
